...
var user = mongoose.model("servers24", yser)
...
exports.addbot = async (userid, token, botid, perfix, owner, kind, days) => {
    return new Promise((resolve, reject) => {
        user.findOne({ userid: userid }).then(async (res) => {
            if (!res) {
                var us = new user({ userid, bots: [{ token, prefix:perfix,botid, owner, kind, DateNow: Date.now(), days: +days }] })
                us.save()
                resolve("done")
...
exports.renew = async(botid, days) => {
      return new Promise((resolve, reject) => {
